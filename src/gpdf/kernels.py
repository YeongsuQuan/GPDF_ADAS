...
@jit
def cdiff(x, y):
    """Difference between each pair of the two collections of row vectors
...
@jit
def cov(x1, x2):
    """Covariance matrix (kernel) between two sets of points
    
    Note
        If L increases, correlation decays slowly with distance.
        If L decreases, correlation decays quickly with distance.
    """
    d = cdist(x1, x2)
...
@jit
def cov_grad(x1, x2):
    """Gradient of the covariance matrix (kernel) between two sets of points"""
...
def cov_hessian(x1, x2):
    """Hessian of the covariance matrix (kernel) between two sets of points"""
    pair_diff_values = cdiff(x1, x2)
    d = jp.linalg.norm(pair_diff_values, axis=2)
